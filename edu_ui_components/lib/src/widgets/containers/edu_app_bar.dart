...
import 'package:edu_ui_components/src/utils/screen_grid.dart';
...
class EduAppBar extends StatefulWidgetWithScrollController implements PreferredSizeWidget {
  ...
  EduAppBar({
    ...
    this.showBorderOnScroll = true,
  }) : preferredSize = _PreferredAppBarSize(toolbarHeight, bottom?.preferredSize.height),
        super(key: key);

  @override
  State<EduAppBar> createState() => _EduAppBarState();
...
class _EduAppBarState extends State<EduAppBar> with ScrollDividerMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final states = <MaterialState>{
      if (settings?.isScrolledUnder ?? scrollDividerOpacity > 0.0) MaterialState.scrolledUnder,
    };
    ...
    final actionForegroundColor = widget.foregroundColor
        ?? appBarTheme.foregroundColor;
    ...
    if (leading == null && widget.automaticallyImplyLeading) {
      if (hasDrawer) {
        ...
        leading = useCloseButton
          ? EduAppBarCloseButton(color: foregroundColor)
          : EduAppBarBackButton(color: foregroundColor);
      }
    }
    if (leading != null) {
    ...
    return Semantics(
      ...
      child: AnnotatedRegion<SystemUiOverlayStyle>(
        ...
        child: Material(
          ...
          shape: widget.showBorderOnScroll
              ? Border(bottom: BorderSide(
                  color: (theme.dividerTheme.color ?? Colors.black).withOpacity(scrollDividerOpacity),
                  width: theme.dividerTheme.thickness ?? 1.0,
                ))
              : widget.shape ?? appBarTheme.shape ?? defaults.shape,
          child: Semantics(
            explicitChildNodes: true,
            child: appBar,
          ),
...
class EduAppBarBackButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ThemeIconButton(
      ...
      color: color,
      onPressed: onTap ?? () => Navigator.of(context, rootNavigator: rootNavigator).maybePop(),
    );
  }
}
...
class EduAppBarCloseButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ThemeIconButton(
      ...
      color: color,
      onPressed: onTap ?? () => Navigator.of(context, rootNavigator: rootNavigator).maybePop(context),
    );
  }
}
